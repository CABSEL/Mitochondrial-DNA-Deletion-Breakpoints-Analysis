...
%% Regions in monkey mtDNA for which hybridization partition function values were calculated
% do not modify
% --------------
...
Ori_LBR_start_position = 5000;          Ori_LBR_stop_position = 11000;      % 5' or left breakpoints should be between these 2 bounds 

Ori_RBR_start_position = 12000;         Ori_RBR_stop_position = 16450;      % 3' or right breakpoints should be between these 2 bounds
...
main_seed_value = 10 * 10000000;      % seed for random number generator, to ensure reproducible results
...
Breakpoints_FileName = 'MonkeyAging_Breakpoints_Lakshmanan_et_al.xls'; 


% load mtDNA hybridization partition function data
% provide the correct directory where the file is saved in the computer

load M://'Mixture Model'/Dec28_2020_Data_MixtureModel/Monkey/Monkey_Complete_MD_Propensity_DS.mat 

% output file name
% user can specify their desired filename for "Dataset_Name"
% no need to specify file format

Dataset_Name = [ 'MonkeyAging','-W',num2str(window_length_val),'-BW', num2str(bin_width),'-SS',num2str(total_sample_sets)]; % name includes details about dataset, window length, bin size, number of samples 
...
for boundary_combination_loop = 1:1:total_boundary_combinations  
    ...
    for MD_component_number = 1:1:total_MD_components
        ...
        for n_ref = 1:1:length(Monkey_Complete_PropMatrix_DS)
    
            if( strcmp( component_ID_val, Monkey_Complete_PropMatrix_DS(n_ref).MD_Component_ID ) == 1 )
        ...
        F_MDcomponent_Propensity_Matrix = [];   F_MDcomponent_Propensity_Matrix = Monkey_Complete_PropMatrix_DS(component_index_in_reference).MD_Component_AvgPF_PropensityMatrix;   % select between AvgPF and MaxPF in this step
