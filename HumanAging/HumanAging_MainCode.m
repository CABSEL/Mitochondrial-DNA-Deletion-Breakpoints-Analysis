%% Code description and usage instructions
   % Description: Code to perform mixture model analysis for one dataset. 
   ...
   % Requirements:  (1) optimization toolbox, (2) ancillary functions used within the code should be present in the same folder
...
%% Regions in human mtDNA for which hybridization partition function values were calculated
% do not modify
% --------------

Ori_LBR_start_position = 5000;          Ori_LBR_stop_position = 11300;     % 5' or left breakpoints should be between these 2 bounds 

Ori_RBR_start_position = 11500;         Ori_RBR_stop_position = 16465;     % 3' or right breakpoints should be between these 2 bounds
...
main_seed_value = 1 * 10000000;       % seed for random number generator, to ensure reproducible results
...
% --------------------------------------------
    % see example data file('HumanAging_BreakpointsData.xls') in the folder for correct input format 
    % file should be in excel2003 format
    
Breakpoints_FileName = 'HumanAging_BreakpointsData.xls'; 


% load human mtDNA hybridization partition function data
% provide the correct directory where the file is saved in the computer

load M://'Mixture Model'/Dec28_2020_Data_MixtureModel/Human/Human_Complete_MD_Propensity_DS.mat 

% output file name
% user can specify their desired filename for "Dataset_Name"
% no need to specify file format

dataset_name_tag = 'HumanAging';

Dataset_Name = [ dataset_name_tag,'-W',num2str(window_length_val),'-BW', num2str(bin_width),'-SS',num2str(total_sample_sets)]; % name includes details about dataset, window length, bin size, number of samples 
...
for boundary_combination_loop = 1:1:total_boundary_combinations  
    ...
    for MD_component_number = 1:1:total_MD_components
        ...
        for n_ref = 1:1:length(Human_Complete_PropMatrix_DS)
    
            if( strcmp( component_ID_val, Human_Complete_PropMatrix_DS(n_ref).MD_Component_ID ) == 1 )
        ...
        F_MDcomponent_Propensity_Matrix = [];   F_MDcomponent_Propensity_Matrix = Human_Complete_PropMatrix_DS(component_index_in_reference).MD_Component_AvgPF_PropensityMatrix;   % select between AvgPF and MaxPF in this step
