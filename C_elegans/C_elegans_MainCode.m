...
%% Regions in mtDNA for which hybridization partition function values were calculated
% do not modify
% --------------
Ori_LBR_start_position = 1000;          Ori_LBR_stop_position = 11000;

Ori_RBR_start_position = 2000;         Ori_RBR_stop_position = 11500;
...
main_seed_value = 16 * 10000000;     % seed for random number generator, to ensure reproducible results
...
Breakpoints_FileName = 'C_elegans_Breakpoints_Lakshmanan_et_al.xls'; 


% load mtDNA hybridization partition function data
% provide the correct directory where the file is saved in the computer

load M://'Mixture Model'/Dec28_2020_Data_MixtureModel/Nematode/Worm_Complete_MD_Propensity_DS.mat 

% output file name
% user can specify their desired filename for "Dataset_Name"
% no need to specify file format

Dataset_Name = [ 'C_elegans','-W',num2str(window_length_val),'-BW', num2str(bin_width),'-SS',num2str(total_sample_sets)]; % name includes details about dataset, window length, bin size, number of samples 
...
for boundary_combination_loop = 1:1:total_boundary_combinations  
    ...
    if( RBR_start_position <= LBR_stop_position ) 
    
        fprintf('overlapping LBR and RBR regions\n');

        row_index = (RBR_start_position - LBR_start_position + 1) - 200 + 1;

        total_LD_ZV_rows = LBR_length - row_index + 1;

        total_zero_value_elements = 0;

        for n = 1:1:total_LD_ZV_rows

            total_zero_value_elements = total_zero_value_elements + n;

        end

        total_submatrix_elements = LBR_length * RBR_length;

        non_LD_ZV_elements = total_submatrix_elements - total_zero_value_elements;

        MI_probability_value_per_bp = 0;    MI_probability_value_per_bp = 1/non_LD_ZV_elements;

        MI_Unbinned_PDF_Marix = [];         MI_Unbinned_PDF_Marix = MI_probability_value_per_bp * ones(LBR_length,RBR_length);

        column_count = 0;

        for row_count = row_index:1:LBR_length

            column_count = column_count + 1;

            for column_loop = 1:1:column_count

                MI_Unbinned_PDF_Marix(row_count,column_loop) = 0;
        ...
        %fprintf('Non-overlapping LBR and RBR regions\n');
    ...
    for MD_component_number = 1:1:total_MD_components
        ...
        for n_ref = 1:1:length(Worm_Complete_PropMatrix_DS)
    
            if( strcmp( component_ID_val, Worm_Complete_PropMatrix_DS(n_ref).MD_Component_ID ) == 1 )
        ...
        F_MDcomponent_Propensity_Matrix = [];   F_MDcomponent_Propensity_Matrix = Worm_Complete_PropMatrix_DS(component_index_in_reference).MD_Component_AvgPF_PropensityMatrix;   % select between AvgPF and MaxPF in this step
    ...
    Components_binnedPDF_DS = WAO_Binned_PDF_calculation_function( Bin_Boundary_Matrix, MI_component_status, MI_Unbinned_PDF_Marix, MDcomponents_DS, LBR_start_position, RBR_start_position); 
...
y_axis_min = 0;             y_axis_min = min( [Prediction_LikelihoodValues;  -fval_optimum ] );            y_axis_min = y_axis_min + ( 0.05 * y_axis_min );

y_axis_max = 0;             y_axis_max = max( [Prediction_LikelihoodValues;  -fval_optimum ] );            y_axis_max = y_axis_max - (0.05 * y_axis_max );
