function[BP_Probability_DataStructure] = Binned_BP_probability_function( Deletion_Breakpoints_Matrix, Components_binnedPDF_DS, Bin_Boundary_Matrix )
    ...
    for component_number = 1:1:total_mixture_components
        ...
        for deletion_number = 1:1:total_deletions_count
            ...
            bin_index = find( ( Deletion_Breakpoints_Matrix(deletion_number,2) >=  Bin_Boundary_Matrix(:,1) ) & ( Deletion_Breakpoints_Matrix(deletion_number,2) <= Bin_Boundary_Matrix(:,2) ) & ( Deletion_Breakpoints_Matrix(deletion_number,3) >= Bin_Boundary_Matrix(:,3) ) & ( Deletion_Breakpoints_Matrix(deletion_number,3) <= Bin_Boundary_Matrix(:,4) ) );
