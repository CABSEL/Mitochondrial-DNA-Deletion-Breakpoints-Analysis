...
Ori_LBR_start_position = 4500;          Ori_LBR_stop_position = 11500;

Ori_RBR_start_position = 11700;         Ori_RBR_stop_position = 16200;
...
main_seed_value = 11 * 10000000;      % seed for random number generator, to ensure reproducible results
...
Breakpoints_FileName = 'RatAging_Breakpoints_Lakshmanan_et_al.xls'; 
...
load M://'Mixture Model'/Dec28_2020_Data_MixtureModel/Rat/Rat_Complete_MD_Propensity_DS.mat 

% output file name
% user can specify their desired filename for "Dataset_Name"
% no need to specify file format

Dataset_Name = [ 'RatAging','-W',num2str(window_length_val),'-BW', num2str(bin_width),'-SS',num2str(total_sample_sets)]; % name includes details about dataset, window length, bin size, number of samples 
...
for boundary_combination_loop = 1:1:total_boundary_combinations  
    ...
    for MD_component_number = 1:1:total_MD_components
        ...
        for n_ref = 1:1:length(Rat_Complete_PropMatrix_DS)
    
            if( strcmp( component_ID_val, Rat_Complete_PropMatrix_DS(n_ref).MD_Component_ID ) == 1 )
        ...
        F_MDcomponent_Propensity_Matrix = [];   F_MDcomponent_Propensity_Matrix = Rat_Complete_PropMatrix_DS(component_index_in_reference).MD_Component_AvgPF_PropensityMatrix;   % select between AvgPF and MaxPF in this step
